...
<template>
    <header class="fixed w-full z-50 transition-all duration-500 ease-in-out">
        ...
        <!-- Top Bordeaux Bar -->
        <div class="bg-[#6C1C23] text-white px-4 lg:px-8 py-2 flex justify-between items-center transition-all duration-500 "
            ...
            <nav class="hidden lg:flex items-center gap-8 transition-opacity duration-300"
                :class="isScrolled ? 'opacity-100 h-16' : 'opacity-0 pointer-events-none w-0 h-0 overflow-hidden'">

                <NuxtLink v-for="link in navLinks" :key="link.id" class="scroll-link hover:text-[#C4A55F]"
            ...
            <div class="flex items-center gap-4">
                ...
                <!-- Mobile Toggle -->
                <button class="lg:hidden text-white" @click="toggle">
                    <XIcon v-if="isOpen" />
                    <MenuIcon v-else />
                </button>
            </div>
        ...
        <div class="bg-white overflow-hidden transition-all duration-500 ease-in-out border-b border-gray-100"
            ...
            <div class="flex items-center">
                <div class="container mx-auto px-4 text-center">
                    <h1 class="text-3xl lg:text-5xl text-black tracking-wide">
                        Sacré-Cœur de Breteuil
                    </h1>
                </div>
                <button class="mr-4 lg:hidden" @click="toggle">
                    <XIcon v-if="isOpen" />
                    <MenuIcon v-else />
                </button>
            </div>
        ...
        <nav class="hidden lg:flex justify-center items-center gap-12 bg-white py-4 border-b border-gray-100 transition-all duration-500 overflow-hidden h-12"
        ...
        <!-- Mobile Drawer -->
        <div v-show="isOpen" class="lg:hidden bg-white shadow-xl py-6 transition-all duration-300">
